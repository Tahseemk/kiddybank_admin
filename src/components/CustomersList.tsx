...
import { getDateWithoutTime, getFormatedDate } from "../Helper";
...
function CustomersList(props: MyComponentProps) {
  ...
  const getCustomerListData = async () => {
    try {
      const payload = { search: searchUser };
      setLoading(true);
      const response = await authAxios().post(
        `admin/listUser/${itemsPerPage}/${currentPage}`,
        payload
      );
      setLoading(false);
      if (response.data.status === "true") {
        const resData = response.data.data;
        setCustomers(resData?.docs || []);
                ...
                <div className="nk-block-between">
                  <div className="nk-block-head-content">
                    ...
                    <div className="search--customer">
                      ...
                      />
                      <button onClick={getCustomerListData} className="btn">
                        Search
                      </button>
                    </div>
                  </div>
...
export default IsLoadingHOC(CustomersList);
